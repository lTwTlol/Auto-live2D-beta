...
1. ブラウザで `index.html` を直接開きます。
2. パーツ分けPSDをドロップ（または「sample.psd を読み込む」）。
...
ブラウザ版に加えて Python デスクトップ版を同梱しています。UI・レイアウトはブラウザ版と同一です。

### 使い方

- ルート直下の `run.bat` をダブルクリックするだけで起動します。初回にプロジェクト専用の `.venv` を作成し、依存関係をその中へインストールします。システムの Python は仮想環境の作成にのみ使用されます（Python 3.10+ が必要）。
- 要件: Windows の Edge WebView2（Win11 には標準搭載）。
- 起動するとデスクトップウィンドウが開き、ブラウザ版と同様に PSD をドロップできます。

### OpenSeeFace 追従

デスクトップ版はウェブカメラ追従に加えて OpenSeeFace 追従（頭部・まばたき・口・虹彩/視線も含む）にも対応しています。`opennseeface/Binary` ディレクトリに入り、`run.bat` をダブルクリック実行します。カメラ一覧が表示されるので、自分のカメラ番号と各パラメータ（モード・FPS）を選ぶと追従が始まります。その後アプリに戻り「自動」セクションの **OpenSeeFace** トグルを有効にすると、`127.0.0.1:11573` の UDP ストリームを読み取ってアバターを動かします。

### 言語切替（3言語）

右上の言語プルダウン（日本語 / English / 简体中文）で即時切替。選択はローカルに保存され、次回起動時も保持されます。
...
表情プリセット（笑顔/驚き/ジト目/ウインク左右）、左右独立の目の開き、眉角度（左右独立+対称）、視線、瞳スケール、目/口の「閉じやすさ」しきい値、前髪3ブロック操作と**前髪専用の揺れ・柔らかさ**、腕の高さ/位置、胸の揺れ（強さ・位置調整可）、体の傾き、アイドル/ランダムモーション、ランダム口パク、マイク口パク、マウス追従、**ウェブカメラ追従**（頭XYZ・まばたき左右・口・視線）、背景切替（透過/グリーンバック）、**3言語UI**（日本語/English/简体中文）。

## 構成

```
index.html       アプリ本体（UI + WebGLランタイム + i18n）
...
main.py          Python デスクトップ版エントリ（pywebview）
requirements.txt Python 版の依存
