...
ブラウザ版に加えて Python デスクトップ版（`main.py`）を同梱しています。UI・レイアウトはブラウザ版と同一です。
...
デスクトップ版はウェブカメラ追従に加えて OpenSeeFace 追従にも対応しています。「自動」セクションの **OpenSeeFace** トグルを有効にすると、`127.0.0.1:11573` の UDP ストリームを読み取ります。先に同じマシンでデータソースを起動してください（同梱の `opennseeface/Binary/facetracker.exe`、または VSeeFace が同ポートへ VMC 送信）。
